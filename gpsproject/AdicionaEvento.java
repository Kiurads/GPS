            ...
            if(!nome.getText().trim().equals("") && data.getValue() != null && tipo.getValue() != null) {
                if(notificar.isSelected()) {
                    if (tempo.getValue() != null) {
                        veiculo.CriarEvento(new Evento(data.getValue(), nome.getText(), veiculo.getMatricula(), (TipoEvento) tipo.getValue(), (String) tempo.getValue()));
                        window.close();
                    } else {
                        Alert.display("Erro", "Tem que especificar o tempo da notificação");
                    }
                } else {
                    veiculo.CriarEvento(new Evento(data.getValue(), nome.getText(), veiculo.getMatricula(), (TipoEvento) tipo.getValue()));
                    window.close();
                }
            } else {
